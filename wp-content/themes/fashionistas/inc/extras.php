...
function athemes_continue_reading_link() {
	return ' <a href="'. esc_url( get_permalink() ) . '" class="more-link">' . __( 'Continue Reading <span class="meta-nav">&rarr;</span>', 'fashionistas' ) . '</a>';
}
...
function athemes_wp_title( $title, $sep ) {
	...
	if ( $paged >= 2 || $page >= 2 )
		$title .= " $sep " . sprintf( __( 'Page %s', 'fashionistas' ), max( $paged, $page ) );
